...
!
  ...
  TYPE, PUBLIC :: DgnState
     ...
     REAL(f4),           POINTER :: OrganicF(:,:,:)
     LOGICAL                     :: Archive_OrganicF
...
!###    State_Diag%Archive_WashFracLS                  = .FALSE.
    ...
    State_Diag%Fy                                  => NULL()
    State_Diag%Archive_Fy                          = .FALSE.
    
    State_Diag%OrganicCl                           => NULL()
    State_Diag%Archive_OrganicCl                   = .FALSE.
    
    State_Diag%OrganicF                            => NULL()
    State_Diag%Archive_OrganicF                    = .FALSE.
...
!
 ...
 10 FORMAT( /, 'Allocating the following fields of the State_Diag object:' )
    ...
    CALL Init_and_Register(                                                  &
         ...
         DiagList       = Diag_List,                                         &
         TaggedDiagList = TaggedDiag_List,                                   &
         Ptr2Data       = State_Diag%Fy,                                    &
         archiveData    = State_Diag%Archive_Fy,                            &
         diagId         = diagId,                                            &
         RC             = RC                                                )
    ...
    CALL Init_and_Register(                                                  &
         ...
         DiagList       = Diag_List,                                         &
         TaggedDiagList = TaggedDiag_List,                                   &
         Ptr2Data       = State_Diag%OrganicF,                               &
         archiveData    = State_Diag%Archive_OrganicF,                       &
         diagId         = diagId,                                            &
         RC             = RC                                                )
...
!
    ...
    ELSE IF ( TRIM( Name_AllCaps ) == 'FY' ) THEN
       IF ( isDesc    ) Desc  = &
            'Inorganic_chlorine_=_F_3NF3_6SF6_2SO2F2'
       IF ( isUnits   ) Units = 'mol mol-1'
       IF ( isRank    ) Rank  = 3

    ELSE IF ( TRIM( Name_AllCaps ) == 'ORGANICCL' ) THEN
       IF ( isDesc    ) Desc  = &
            '4CCl4_H1211_3CFC11_3CFC13_3CFC113a_3CFC113_2CFC114_CFC115_2CFC12_3CH3CCl3_CH3Cl_2HCFC141b_HCFC142b_HCFC22_2HCFC123_HCFC124_HCFC133a_2HCFC132b_HCFC31_3CHCl3_2CH2Cl2_CH2ICl'
       IF ( isUnits   ) Units = 'mol mol-1'
       IF ( isRank    ) Rank  = 3
       
    ELSE IF ( TRIM( Name_AllCaps ) == 'ORGANICF' ) THEN
       IF ( isDesc    ) Desc  = &
            '4HFC134a_5HFC125_3HFC143a_2HFC152a_2HFC32_3HFC23_7HFC227ea_6HFC236fa_5HFC245fa_5HFC365mfc_10HFC4310mee_4CF4_6C2F6_8C3F8_8C4F8_10C4F10_12C5F12_14C6F14'
       IF ( isUnits   ) Units = 'mol mol-1'
       IF ( isRank    ) Rank  = 3
