...
c-----------------------------------------------------------------------
      subroutine XNF3 (WW,XT,XP,XM,XNEW,
     &     INIT, MM,TTT,PPP,ISX,ISP,TITLNEW,TITLTBL)
c-----------------------------------------------------------------------
...
c---NB NF3 Xsections scaling valid for 212-296 K and 184.95-250 nm only
c---apply min-max range for T dependence: set for NF3
        WWL = min(250.d0, max(184.95, WW))
        XBFACT = XB(1) + XB(2)*WWL + XB(3)*(WWL**2) + XB(4)*(WWL**3) + 
     &   XB(5)*(WWL**4)
        TTL = min(296.d0, max(212.d0, XT))
        XTFACT = 10**( (TTL-296.d0)*XBFACT)
