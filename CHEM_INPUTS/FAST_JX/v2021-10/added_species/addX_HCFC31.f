...
!!!!!!!!!!!!!!!!!!initialization call to user subroutine!!!!!!!!!!!!!!
      ...
      call XHCFC31 (W(1),XT,XP,XM, XNEW,
...
! primary high-resolution wavelength loop - generate input for pratmo reference J's
      ...
      call XHCFC31 (W(J),XT,XP,XM, XNEW,
     &      INIT, MM,TTT,PPP,ISX,ISP,TITLNEW,TITLTBL)
...
c-----------------------------------------------------------------------
      subroutine XHCFC31 (WW,XT,XP,XM,XNEW,
     &     INIT, MM,TTT,PPP,ISX,ISP,TITLNEW,TITLTBL)
c-----------------------------------------------------------------------
...
c---NB HCFC31 Xsections scaling valid for 208-298 K and 160-230 nm only
c---apply min-max range for T dependence: set for HCFC31
        WWL = min(230.d0, max(160.d0, WW))
        XBFACT = 0.d0
        TTL = min(298.d0, max(208.d0, XT))
        XTFACT = exp((TTL-298.d0)*XBFACT)
