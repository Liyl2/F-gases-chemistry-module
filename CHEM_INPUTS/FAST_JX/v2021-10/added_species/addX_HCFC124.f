...
!!!!!!!!!!!!!!!!!!initialization call to user subroutine!!!!!!!!!!!!!!
      ...
      call XHCFC124 (W(1),XT,XP,XM, XNEW,
...
! primary high-resolution wavelength loop - generate input for pratmo reference J's
      ...
      call XHCFC124 (W(J),XT,XP,XM, XNEW,
     &      INIT, MM,TTT,PPP,ISX,ISP,TITLNEW,TITLTBL)
...
c-----------------------------------------------------------------------
      subroutine XHCFC124 (WW,XT,XP,XM,XNEW,
     &     INIT, MM,TTT,PPP,ISX,ISP,TITLNEW,TITLTBL)
c-----------------------------------------------------------------------
...
c---NB HCFC124 Xsections scaling valid for 210-300 K and 170-230 nm only
c---apply min-max range for T dependence: set for HCFC124
        WWL = min(230.d0, max(170.d0, WW))
        XBFACT = XB(1) + XB(2)*WWL + XB(3)*(WWL**2) + XB(4)*(WWL**3)
        TTL = min(300.d0, max(210.d0, XT))
        XTFACT = 10**((TTL-273.d0)*XBFACT)
...
c--- XBFACT = XB(1)*WWL + XB(2)*(WWL-212.d0) + XB(3)*(WWL-212.d0)**2
