...
c-----------------------------------------------------------------------
      subroutine XCF3Cl (WW,XT,XP,XM,XNEW,
     &     INIT, MM,TTT,PPP,ISX,ISP,TITLNEW,TITLTBL)
c-----------------------------------------------------------------------
...
c---NB CF3Cl Xsections scaling valid for 210-300 K and 172-200 nm only
c---apply min-max range for T dependence: set for CF3Cl
        WWL = min(200.d0, max(172.d0, WW))
        XBFACT = 0.d0
...
c--- XBFACT = XB(1) + XB(2)*WWL + XB(3)*(WWL**2) + XB(4)*(WWL**3)
