...
!!!!!!!!!!!!!!!!!!initialization call to user subroutine!!!!!!!!!!!!!!
      ...
      call XHCFC133a (W(1),XT,XP,XM, XNEW,
...
! primary high-resolution wavelength loop - generate input for pratmo reference J's
      ...
      call XHCFC133a (W(J),XT,XP,XM, XNEW,
     &      INIT, MM,TTT,PPP,ISX,ISP,TITLNEW,TITLTBL)
...
c-----------------------------------------------------------------------
      subroutine XHCFC133a (WW,XT,XP,XM,XNEW,
     &     INIT, MM,TTT,PPP,ISX,ISP,TITLNEW,TITLTBL)
c-----------------------------------------------------------------------
...
c---NB HCFC133a Xsections scaling valid for 208-298 K and 160-245 nm only
c---apply min-max range for T dependence: set for HCFC133a
        WWL = min(245.d0, max(160.50, WW))
        XBFACT = XB(1) + XB(2)*WWL + XB(3)*(WWL**2) + XB(4)*(WWL**3) + 
     &   XB(5)*(WWL**4) + XB(6)*(WWL**5)
        TTL = min(298.d0, max(208.d0, XT))
