...
!!!!!!!!!!!!!!!!!!initialization call to user subroutine!!!!!!!!!!!!!!
      ...
      call XCFC113a (W(1),XT,XP,XM, XNEW,
...
! primary high-resolution wavelength loop - generate input for pratmo reference J's
      ...
      call XCFC113a (W(J),XT,XP,XM, XNEW,
     &      INIT, MM,TTT,PPP,ISX,ISP,TITLNEW,TITLTBL)
...
c-----------------------------------------------------------------------
      subroutine XCFC113a (WW,XT,XP,XM,XNEW,
     &     INIT, MM,TTT,PPP,ISX,ISP,TITLNEW,TITLTBL)
c-----------------------------------------------------------------------
...
c---NB CFC113a Xsections scaling valid for 207-323 K and 192.5-235 nm only
c---apply min-max range for T dependence: set for CFC113a
        WWL = min(235.d0, max(192.50, WW))
        XBFACT = XB(1) + XB(2)*WWL + XB(3)*(WWL**2) + XB(4)*(WWL**3) + 
     &   XB(5)*(WWL**4) + XB(6)*(WWL**5)
        TTL = min(323.d0, max(207.d0, XT))
